                    ...
                    <div class="row">
                        <div class="col-xl-6 mb-4">
                            <select class="form-select wide" id="bulan_tahun" name="bulan_tahun">
                                @php
                                    $dataFirstBulanTahun = $periods->first();
                                @endphp
                                <option value="{{ $dataFirstBulanTahun->month }}-{{ $dataFirstBulanTahun->year }}">{{ $dataFirstBulanTahun->month_name }}, {{ $dataFirstBulanTahun->year }}</option>
                                @foreach ($periods as $period)
                                <option value="{{ $period->month }}-{{ $period->year }}">{{ $period->month_name }}, {{ $period->year }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-xl-6">
